...
$(document).on('submit', "#editAppBannerForm", function(e){
    e.preventDefault();
    var editButton = document.querySelector("#editButton");
    var editAppBannerForm = $("#editAppBannerForm");
    var form_data = new FormData(this);
    console.log(form_data)
    editAppBannerForm.validate({
        // lang:'ar',
        rules: {
            title: {
                required: true,
    ...
    if (!editAppBannerForm.valid()){
        return;
    }
    loadPreloader(true)
    editButton.setAttribute("data-kt-indicator", "on");
    let id = $("input[name=id]").val();
    var update_url = admin_base_url+"/app_banners/"+id+"/update"
    $.ajax({
        type:'POST',
